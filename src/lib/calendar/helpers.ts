...
import type { ICalendarCell, IEvent } from "@/lib/calendar/interfaces"
...
export const getEventsForDay = (
	...
	return events
		.filter((event) => {
			...
			const startOfDayForEventStart = startOfDay(eventStartDate)
			const startOfDayForEventEnd = startOfDay(eventEndDate)

			// For single day events, simply check if event is on target date
			if (isSameDay(startOfDayForEventStart, startOfDayForEventEnd)) {
				return isSameDay(targetDate, startOfDayForEventStart)
			}

			// For multi-day events
			if (isWeek) {
				// In week view, only include multi-day events
				return (
			...
			// In other views, check if target date matches the start or end date
			return (
				isSameDay(targetDate, startOfDayForEventStart) ||
				isSameDay(targetDate, startOfDayForEventEnd)
			)
		})
		.map((event) => {
