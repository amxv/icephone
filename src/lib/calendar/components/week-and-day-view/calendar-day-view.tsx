...
import { CalendarTimeline } from "@/lib/calendar/components/week-and-day-view/calendar-time-line"
import { DayViewMultiDayEventsRow } from "@/lib/calendar/components/week-and-day-view/day-view-multi-day-events-row"

import { groupEvents } from "@/lib/calendar/helpers"

import { DroppableArea } from "@/lib/calendar/components/dnd/droppable-area"
import { RenderGroupedEvents } from "@/lib/calendar/components/week-and-day-view/render-grouped-events"
...
export function CalendarDayView({ singleDayEvents, multiDayEvents }: IProps) {
	...
	const dayEvents = singleDayEvents.filter((event) => {
		...
		// For day view, only show events that start or end on the selected day
		// For single-day events (starts and ends on same day)
		if (isSameDay(eventStartDate, eventEndDate)) {
			return isSameDay(selectedDate, eventStartDate)
		}

		// For multi-day events, we only want to show them on the start day in day view
		// Multi-day events are shown in the multi-day row at the top
		return isSameDay(selectedDate, eventStartDate)
	})
					...
					{currentEvents.length > 0 ? (
						<div className="flex items-start gap-2 px-4 pt-4">
							<span className="relative mt-[5px] flex size-2.5">
								...
								<span className="relative inline-flex size-2.5 rounded-full bg-success-600" />
							</span>
